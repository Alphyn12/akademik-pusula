...
class DergiParkScraper(BaseScraper):
    ...
    def _fetch_sync(self, query: str, start_year: int, end_year: int) -> List[Dict[str, Any]]:
        results = []
        try:
            dp_query = f"{query} site:dergipark.org.tr"
            search_query = scholarly.search_pubs(dp_query)
            ...
            for i in range(15):
                try:
                    ...
                    status = "Açık" 
                    link = pub_url if pub_url else (eprint_url if eprint_url else "-")
                    ...
                    self.logger.warning(f"Error parsing dergipark item: {item_e}")
                    continue
